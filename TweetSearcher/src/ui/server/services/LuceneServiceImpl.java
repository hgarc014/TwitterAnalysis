...
public class LuceneServiceImpl extends RemoteServiceServlet implements LuceneService {
...
//			System.out.println(new File("indexes/index").getAbsolutePath());

			ireader = IndexReader.open(new SimpleFSDirectory(new File("indexes/index")));
	...
	public List<Tweet> getTweets(String query, String type) {
		...
		// tweets.add(new Tweet("10", "10", "10", "10", "10", "en", "me", test
		// .getAbsolutePath(), "TITLE", "link", "tags", "img"));
		// }
		return tweets;
	}

	private ScoreDoc[] generalSearh(String[] query, String exact) throws IOException,
			ParseException {
	...
	private ScoreDoc[] getRecentTweets() throws IOException {
		...
		sorter.setSort(sf);
		TopFieldDocs tfd = isearcher.search(bq, maxResults, sorter);
		return tfd.scoreDocs;
	}

	private ScoreDoc[] getUser(String[] users) throws IOException, ParseException {
		BooleanQuery bq = new BooleanQuery();
		Query q = null;
		Analyzer anl = new StandardAnalyzer();
		QueryParser pars = null;
	...
	private ScoreDoc[] getHashTags(String[] tags) throws IOException, ParseException {
		BooleanQuery bq = new BooleanQuery();
		Query q = null;
		Analyzer anl = new StandardAnalyzer();
		QueryParser pars = null;
	...
	private Tweet newTweet(Document d) throws java.text.ParseException {
		Tweet t = new Tweet(d.get("created_at"), d.get("favoriteCount"), d.get("retweets"),
				d.get("longitude"), d.get("latitude"), d.get("language"), d.get("user"),
				d.get("text"), d.get("link"), getUrlStringFrom(d.get("text")), d.get("hashtags"),
